...
linha_para_editar = st.number_input("Selecione a linha para editar (índice)", min_value=0, max_value=len(st.session_state.mhd_data) - 1, step=1)
texto_editado = st.text_area("Novo texto para a descrição", st.session_state.mhd_data.iloc[linha_para_editar, 1] if len(st.session_state.mhd_data) > 0 else "")
